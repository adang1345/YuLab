"""Count the number of silent mutations from TCGA and COSMIC that are in disordered interface regions or in
...
with open("../Disordered Region Data/DisorderData.txt") as ddataf:
    ...
    for x in ddataf.readlines():
        xl = x.split()
        # EDIT THIS LINE TO FILTER DISORDER DATA
...
# read cancer mutation data and consider only silent mutations for proteins that show up in the interface data and
# the disorder data
with open("../Mutation Data/CancerMutationData.txt") as mutf:
    mutdata = []
    for x in mutf.readlines():
        xl = x.split()
        if xl[4] == "Silent" and xl[1] in uniprot_interface and xl[1] in uniprot_disorder:
            mutdata.append(xl)
...
print("Silent mutations in disordered interface\t" + str(in_disordered_interface))
print("Silent mutations in structured interface\t" + str(in_structured_interface))
